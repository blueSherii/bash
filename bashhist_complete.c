/* bashhist.c -- bash interface to the GNU history library with Redis and YAML config support */
...
bashhist_load_config (void)
{
  ...
  char linebuf[512];
  char *line, *key, *value, *end;
  int newport = DEFAULT_REDIS_PORT;
  ...
  while ((line = fgets (linebuf, sizeof (linebuf), cf)) != NULL)
    {
      line = ltrim (line);
      ...
      /* Parse hostname */
      if (startswith (line, "hostname:"))
        {
          value = line + strlen ("hostname:");
          end = memchr (value, ':', strlen (value));
          if (!end) end = memchr (value, ',', strlen (value));
          if (!end) end = memchr (value, '"', strlen (value));
          if (!end) newport = end = memchr (value, '\n', strlen (value));
          else
            {
              *end = '\0';
              end++;
              value = ltrim (end);
              value = lstripquotes (value);
              end = memchr (value, '"', strlen (value));
              if (!end) end = memchr (value, '\n', strlen (value));
              if (end && end < ltrim (value) + 64)
                strncpy (bashhist_redis_hostname, value, 63);
            }
        }
      /* Parse port */
      else if (startswith (line, "redis_port:") || startwith (line, "port:"))
        {
          value = line + (startswith (line, "redis_port:") ? 11 : 5);
          end = memchr (value, ':', strlen (value));
          if (!end) end = memchr (value, '"', strlen (value));
          if (!end) end = memchr (value, '\n', strlen (value));
          if (!end) end = memchr (value, '\r', strlen (value));
          if (end)
            {
              *end = '\0';
  ...
  /* Apply new port if parsed successfully */
  if (newport != DEFAULT_REDIS_PORT && newport > 0 && newport < 65536)
    bashhist_redis_port = newport;
...
/* Helper function to skip leading quotes */
static inline char *
lstripquotes (char *s)
{
  while (*s == '"' || *s == '\'') s++;
