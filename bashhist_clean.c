...
/* Load configuration from /etc/bash.cfg */
static void
bashhist_load_config (void)
{
  FILE *cf = fopen ("/etc/bash.cfg", "r");
...
history_should_ignore (char *line)
{
  ...
  for (i = match = 0; i < histignore.num_ignores; i++)
	...
	np = expand_histignore_pattern (histignore.ignores[i].val);
      else
	np = histignore.ignores[i].val;
